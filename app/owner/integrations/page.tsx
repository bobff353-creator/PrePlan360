  ...
  return <main className="access-shell owner-shell">
    ...
    <section className="access-page integration-page">
      ...
      <div className="integration-grid">
        <article className="integration-card"><div className="integration-card-head"><div><span>DIRECT CAD</span><h2>Signed CAD intake</h2></div><Status ready={readiness.cadConfigured}/></div><label>Webhook URL</label><code>{baseUrl}/api/webhooks/cad</code><label>Server-only setting</label><code>CAD_WEBHOOK_SECRET</code><ol><li>Create a long random signing secret in the CAD provider and add the same value to the hosted environment.</li><li>Sign the exact raw JSON body with HMAC-SHA256.</li><li>Send <code>x-preplan-signature: sha256=&lt;hex&gt;</code>, plus a stable correlation ID.</li><li>Verify the event below, then approve department routing and incident rules.</li></ol></article>
        <article className="integration-card"><div className="integration-card-head"><div><span>RESEND</span><h2>Inbound CAD email</h2></div><Status ready={readiness.resendConfigured}/></div><label>Webhook URL</label><code>{baseUrl}/api/webhooks/resend</code><label>Required server-only setting</label><code>RESEND_WEBHOOK_SECRET</code><label>Optional future content retrieval</label><div className="integration-inline"><code>RESEND_API_KEY</code><Status ready={readiness.resendContentConfigured} readyLabel="Available"/></div><ol><li>In Resend, add this URL as a webhook and select <code>email.received</code>.</li><li>Copy that webhook’s signing secret into the hosted environment.</li><li>Use a Resend receiving address or a separate receiving subdomain.</li><li>Metadata is recorded now; body and attachment retrieval stays off until an API key and department rules are approved.</li></ol></article>
      </div>
