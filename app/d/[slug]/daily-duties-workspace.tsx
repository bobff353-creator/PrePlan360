...
type Props = { departmentId: string; data: StickneyModuleData; editable: boolean; supportSessionId: string };
...
export default function DailyDutiesWorkspace({ departmentId, data, editable, supportSessionId }: Props) {
  ...
  return <section className="duties-foundation">
    <div className="duties-source"><div><i/><span><b>Live Stickney Daily Duties</b><small>Real weekly station rotation with audited department overlays</small></span></div><strong>Source preserved</strong></div>
    ...
    <p className="duties-footer">The 21 original recurring duties remain in Stickney Firehouse Manager. Completions, assignments, added duties, and owner edits in this build are stored as audited PrePlan 360 overlays.</p>
  </section>;
}
...
function TodayView({ departmentId, duties, context, formattedDate, editable, supportSessionId }: { departmentId: string; duties: StickneyDuty[]; context: NonNullable<StickneyModuleData["dutyContext"]>; formattedDate: string; editable: boolean; supportSessionId: string }) {
  ...
  return <div className="duties-panel"><header><div><span>TODAY&apos;S DUTIES</span><h2>{dayNames[context.dayOfWeek]} Duty Roster — {formattedDate}</h2><p>Only Stickney&apos;s recorded duties for today are shown.</p></div><b>{done}/{duties.length}</b></header>{duties.length ? <div className="duties-list">{sortDuties(duties).map((duty) => <article key={duty.id} className={duty.completed_date === context.date ? "complete" : ""}><div className="duty-check" aria-label={duty.completed_date === context.date ? "Complete" : "Open"}>{duty.completed_date === context.date ? "✓" : ""}</div><div><b>{duty.duty}</b><span>{[duty.detail, duty.due_time, duty.assigned_to || "Unassigned"].filter(Boolean).join(" · ")}</span><small>{duty.category || "Recurring station duty"}</small></div>{editable ? <div className="duty-actions"><DutyCompletionForm departmentId={departmentId} duty={duty} date={context.date} supportSessionId={supportSessionId}/><details><summary>Edit</summary><DutyForm departmentId={departmentId} duty={duty} supportSessionId={supportSessionId}/></details></div> : null}</article>)}</div> : <Empty title="No duties for today" text="No recurring Stickney duties are assigned to this day."/>}<div className="duties-progress"><span style={{ width: `${percent}%` }}/><b>{percent}% complete</b></div>{editable ? <details className="duties-add"><summary>+ Add duty</summary><DutyForm departmentId={departmentId} duty={null} supportSessionId={supportSessionId} defaultDay={context.dayOfWeek}/></details> : null}</div>;
}
