...
type Props = { departmentId: string; data: StickneyModuleData; sourceName: string; sourceSystem: string; editable: boolean; supportSessionId: string };
...
export default function DailyDutiesWorkspace({ departmentId, data, sourceName, sourceSystem, editable, supportSessionId }: Props) {
  ...
  return <section className="duties-foundation">
    <div className="duties-source"><div><i/><span><b>{sourceName} Daily Duties</b><small>Real station duties with audited department overlays</small></span></div><strong>Source preserved</strong></div>
    ...
    {view === "today" ? <TodayView departmentId={departmentId} sourceName={sourceName} duties={today} context={context} formattedDate={formattedDate} editable={editable} supportSessionId={supportSessionId}/> : null}
    {view === "weekly" ? <WeeklyView departmentId={departmentId} sourceName={sourceName} duties={duties} context={context} editable={editable} supportSessionId={supportSessionId}/> : null}
    {view === "chores" ? <ChoresView departmentId={departmentId} sourceName={sourceName} duties={today} context={context} formattedDate={formattedDate} editable={editable} supportSessionId={supportSessionId}/> : null}
    <p className="duties-footer">The original duty records remain in {sourceSystem}. Completions, assignments, added duties, and owner edits in this build are stored as audited PrePlan 360 overlays.</p>
  </section>;
}

function TodayView({ departmentId, sourceName, duties, context, formattedDate, editable, supportSessionId }: { departmentId: string; sourceName: string; duties: StickneyDuty[]; context: NonNullable<StickneyModuleData["dutyContext"]>; formattedDate: string; editable: boolean; supportSessionId: string }) {
