...
const AssetCapture = dynamic(() => import("./asset-capture"));
...
export default function FleetWorkspace({ departmentId, data, sourceName, sourceSystem, inventoryPhotoRoute, editable, supportSessionId }: Props) {
  ...
  return <section className="fleet-foundation">
    ...
    {view === "assets" ? <AssetsView departmentId={departmentId} inventoryPhotoRoute={inventoryPhotoRoute} apparatus={apparatus} compartments={compartments} photos={photos} checks={checks} sourceName={sourceName} editable={editable} supportSessionId={supportSessionId}/> : null}
...
function AssetsView({ departmentId, inventoryPhotoRoute, apparatus, compartments, photos, checks, sourceName, editable, supportSessionId }: { departmentId: string; inventoryPhotoRoute: string; apparatus: StickneyApparatus[]; compartments: NonNullable<StickneyModuleData["compartments"]>; photos: NonNullable<StickneyModuleData["inventoryPhotos"]>; checks: NonNullable<StickneyModuleData["fleetChecks"]>; sourceName: string; editable: boolean; supportSessionId: string }) {
  const [selected, setSelected] = useState(apparatus[0]?.id || "");
  const unit = apparatus.find((item) => item.id === selected);
  return <div className="fleet-panel">
    {editable ? <AssetCapture departmentId={departmentId} supportSessionId={supportSessionId}/> : <div className="asset-view-only"><b>View-only asset workspace</b><span>A department administrator can scan and save apparatus or equipment.</span></div>}
    <div className="fleet-assets"><section className="fleet-capture"><span>DEPARTMENT ASSET REGISTER</span><h2>Reviewed department records</h2><p>Scanned additions save only to this department. Imported {sourceName} apparatus stays unchanged.</p><a href="#native-assets" onClick={() => { const details = document.getElementById("native-assets") as HTMLDetailsElement | null; if (details) details.open = true; }}>Open saved assets, manuals, and maintenance</a></section><section className="fleet-intelligence"><span>MAINTENANCE INTELLIGENCE</span><h2>Build the service record</h2><p>Store reviewed VIN identity, manuals, parts catalogs, vendors, service schedules, due dates, and repair notes. Human verification is required before operational use.</p><div><span>Official manuals<small>Reviewed manufacturer link</small></span><span>Parts catalogs<small>Ordering and diagram source</small></span><span>Maintenance guidance<small>Verified interval and due day</small></span><span>Repair information<small>Work orders and history</small></span></div></section><aside><header><span>IMPORTED APPARATUS</span><b>{apparatus.length}</b></header>{apparatus.map((item) => <button className={item.id === selected ? "active" : ""} key={item.id} onClick={() => setSelected(item.id)}><i className={item.status}/><span><b>{item.name}</b><small>{item.asset_type}</small></span><em>{label(item.status)}</em></button>)}</aside><main>{unit ? <><header><div><span>{unit.asset_type}</span><h2>{unit.name}</h2><p>{[unit.year,unit.manufacturer,unit.model].filter(Boolean).join(" ") || "Vehicle details not entered"}</p></div><em className={unit.status === "in_service" ? "ready" : "danger"}>{label(unit.status)}</em></header>{photos.find((photo) => photo.apparatus_id === unit.id) ? <Image unoptimized width={640} height={360} src={`/api/departments/${departmentId}/${inventoryPhotoRoute}/${photos.find((photo) => photo.apparatus_id === unit.id)!.id}`} alt={`${unit.name} inventory view`}/> : null}<dl><div><dt>VIN</dt><dd>{unit.vin || "Not entered"}</dd></div><div><dt>Compartments</dt><dd>{compartments.filter((item) => item.apparatus_id === unit.id).length}</dd></div><div><dt>Photos</dt><dd>{photos.filter((item) => item.apparatus_id === unit.id).length}</dd></div><div><dt>Checks</dt><dd>{checks.filter((item) => item.apparatus_id === unit.id).length}</dd></div><div><dt>Weekly due</dt><dd>{unit.weekly_due_day == null ? "Not scheduled" : dayNames[unit.weekly_due_day]}</dd></div><div><dt>Last odometer</dt><dd>{checks.find((item) => item.apparatus_id === unit.id && item.latest_odometer != null)?.latest_odometer?.toLocaleString() || "Not recorded"}</dd></div></dl>{editable ? <details className="fleet-edit"><summary>Edit imported apparatus overlay</summary><ApparatusForm departmentId={departmentId} unit={unit} supportSessionId={supportSessionId}/></details> : null}</> : <Empty title="No apparatus selected" text="Choose a unit from the apparatus list."/>}</main></div>
  </div>;
}
