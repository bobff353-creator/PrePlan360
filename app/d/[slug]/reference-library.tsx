...
function SharedPreplanCard({ record, hydrants }: { record: SharedPreplan; hydrants: SharedHydrant[] }) {
  const points = footprint(record.footprint_json);
  return <details className="reference-card reference-preplan-card external" id={`shared-preplan-${record.id}`}><summary><div className="reference-card-top"><div><small>{record.department_name}</small><b>{record.property_name}</b><span>{record.address}</span></div><span className="view-only-lock">View only</span></div><div className="reference-facts"><span>{record.latitude && record.longitude ? points.length >= 3 ? "Mapped footprint" : "Location only" : "Not mapped"}</span><span>Open full property workspace</span></div></summary><div className="reference-card-body"><PreplanDetail editable={false} shared record={{ name: record.property_name, address: record.address, status: record.status, sourceLabel: record.department_name, construction: "", floors: "", fireFlowGpm: null, access: "", alarm: "", sprinkler: "", fdc: "", knox: "", riser: "", summary: record.operational_summary, contact: "", internalNotes: "", latitude: numberOrNull(record.latitude), longitude: numberOrNull(record.longitude), footprintCount: points.length, lastReviewed: record.last_reviewed || "", visibility: record.visibility, updatedAt: record.updated_at }} hydrants={nearbyHydrants(record.latitude, record.longitude, hydrants)}/><footer><span>{record.last_reviewed ? `Reviewed ${record.last_reviewed}` : "Review date not published"}</span></footer></div></details>;
}
