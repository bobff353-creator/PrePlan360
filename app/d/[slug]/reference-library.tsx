...
function nearbyHydrants(latitude: string, longitude: string, hydrants: Array<DepartmentHydrant | SharedHydrant>): PreplanDetailHydrant[] {
  const lat = numberOrNull(latitude);
  const lng = numberOrNull(longitude);
  if (lat == null || lng == null) return [];
  return hydrants
    .map((hydrant) => ({ hydrant, lat: numberOrNull(hydrant.latitude), lng: numberOrNull(hydrant.longitude) }))
    .filter((item): item is { hydrant: DepartmentHydrant | SharedHydrant; lat: number; lng: number } => item.lat != null && item.lng != null)
    .sort((a, b) => (a.lat - lat) ** 2 + (a.lng - lng) ** 2 - ((b.lat - lat) ** 2 + (b.lng - lng) ** 2))
    .slice(0, 5)
    .map(({ hydrant }) => ({ name: hydrant.hydrant_number, location: hydrant.location, status: hydrant.status.replaceAll("_", " "), flowGpm: hydrant.flow_gpm }));
}
...
export default function ReferenceLibrary(props: Props) {
  ...
  return <div className="reference-workspace">
    ...
    <div className={`reference-columns ${isPreplans ? "preplan-columns" : ""}`}>
...
function OwnPreplan({ record, department, hydrants, editable, supportSessionId }: { record: DepartmentPreplan; department: Department; hydrants: DepartmentHydrant[]; editable: boolean; supportSessionId: string }) {
  const points = footprint(record.footprint_json);
  return <details className="reference-card reference-preplan-card" id={`preplan-${record.id}`}><summary><span className="reference-card-top"><span className="reference-card-identity"><b>{record.property_name}</b><span>{record.address}</span></span><Visibility value={record.visibility}/></span><span className="reference-facts"><span>{record.latitude && record.longitude ? points.length >= 3 ? "Mapped footprint" : "Location only" : "Not mapped"}</span><span>Open full property workspace</span></span></summary><div className="reference-card-body"><PreplanDetail editable={editable} record={{ name: record.property_name, address: record.address, status: record.status, sourceLabel: department.name, construction: "", floors: "", fireFlowGpm: null, access: "", alarm: "", sprinkler: "", fdc: "", knox: "", riser: "", summary: record.operational_summary, contact: "", internalNotes: record.internal_notes, latitude: numberOrNull(record.latitude), longitude: numberOrNull(record.longitude), footprintCount: points.length, lastReviewed: record.last_reviewed || "", visibility: record.visibility, updatedAt: record.updated_at }} hydrants={nearbyHydrants(record.latitude, record.longitude, hydrants)}/>{editable ? <details className="reference-map-edit"><summary>Edit map location</summary><form method="post" action={`/api/departments/${department.id}/preplans/${record.id}`}><input type="hidden" name="support_session_id" value={supportSessionId}/><input type="hidden" name="mode" value="map"/><label>Verified latitude<input inputMode="decimal" name="latitude" defaultValue={record.latitude}/></label><label>Verified longitude<input inputMode="decimal" name="longitude" defaultValue={record.longitude}/></label><label className="wide">Footprint corners<textarea name="footprint" defaultValue={footprintText(record.footprint_json)} placeholder="One latitude, longitude pair per line"/></label><button type="submit">Save map</button></form></details> : null}<footer><span>{record.last_reviewed ? `Reviewed ${record.last_reviewed}` : "Review date not set"}</span>{editable ? <ShareForm action={`/api/departments/${department.id}/preplans/${record.id}`} visibility={record.visibility} supportSessionId={supportSessionId}/> : null}</footer></div></details>;
}
...
function SharedPreplanCard({ record, hydrants }: { record: SharedPreplan; hydrants: SharedHydrant[] }) {
  const points = footprint(record.footprint_json);
  return <details className="reference-card reference-preplan-card external" id={`shared-preplan-${record.id}`}><summary><span className="reference-card-top"><span className="reference-card-identity"><small>{record.department_name}</small><b>{record.property_name}</b><span>{record.address}</span></span><span className="view-only-lock">View only</span></span><span className="reference-facts"><span>{record.latitude && record.longitude ? points.length >= 3 ? "Mapped footprint" : "Location only" : "Not mapped"}</span><span>Open full property workspace</span></span></summary><div className="reference-card-body"><PreplanDetail editable={false} shared record={{ name: record.property_name, address: record.address, status: record.status, sourceLabel: record.department_name, construction: "", floors: "", fireFlowGpm: null, access: "", alarm: "", sprinkler: "", fdc: "", knox: "", riser: "", summary: record.operational_summary, contact: "", internalNotes: "", latitude: numberOrNull(record.latitude), longitude: numberOrNull(record.longitude), footprintCount: points.length, lastReviewed: record.last_reviewed || "", visibility: record.visibility, updatedAt: record.updated_at }} hydrants={nearbyHydrants(record.latitude, record.longitude, hydrants)}/><footer><span>{record.last_reviewed ? `Reviewed ${record.last_reviewed}` : "Review date not published"}</span></footer></div></details>;
}
