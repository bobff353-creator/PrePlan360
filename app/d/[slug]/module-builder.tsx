...
function RespondWorkspace({ moduleName, heading, description, instructions, action, data, editable, supportSessionId }: { moduleName: string; heading: string; description: string; instructions: string; action: string; data: DepartmentModuleData; editable: boolean; supportSessionId: string }) {
  ...
  return <section className={`respond-department-workspace${activeIncident ? " incident-active" : " standby"}`}>
    ...
    <div className="respond-department-grid">
      ...
      <RespondPanel label="Preplans and resources" title={`${resources.length} connected record${resources.length === 1 ? "" : "s"}`} tone="ready" items={resources} empty="No department-approved preplan or response resource is linked yet."/>
