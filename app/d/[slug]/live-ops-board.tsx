...
export default function LiveOpsBoard({ departmentId, departmentSlug, departmentName, weatherLocation, vehicleCount, settings, data, sourceData, assets, editable, supportSessionId, saveStatus }: Props) {
  ...
  useEffect(() => {
    ...
    return () => window.clearInterval(timer);
  }, [activeIncident, departmentName, settings.live_board_radar_display_seconds, settings.live_board_radar_refresh_minutes, weather?.configured]);
  ...
  return <section className="live-ops-foundation">
    ...
    {takeover && !activeIncident ? <RadarOverlay departmentId={departmentId} takeover={takeover} radarUrl={settings.live_board_radar_url} refreshKey={takeover.endsAt} clock={clock} onClose={() => setTakeover(null)}/> : null}
  </section>;
}
...
function WeatherHeader({ departmentName, weather, index, clock, rotationSeconds, editable, settingsOpen, onSettings }: { departmentName: string; weather: WeatherPayload | null; index: number; clock: Date | null; rotationSeconds: number; editable: boolean; settingsOpen: boolean; onSettings: () => void }) {
  ...
  return <header className="live-weather-head"><div className="live-weather-copy"><span>{eyebrow}</span><h2>{title}</h2><p>{detail}</p></div><div className="live-ops-head-actions">{editable ? <button className="board-customize-control" type="button" onClick={onSettings}>{settingsOpen ? "Close settings" : "Customize board"}</button> : null}<div className="live-ops-clock"><b>{militaryTime(clock)}</b><span>{clock ? clock.toLocaleDateString([], { weekday: "long", month: "long", day: "numeric" }) : ""}</span><small><i/> Rotates every {rotationSeconds}s</small></div></div></header>;
}
...
function StationWidget({ departmentId, panel, items, settings, weather, lodd, sourceData, departmentSlug, refreshKey, index, onSelect }: { departmentId: string; panel: FoundationSettings["live_board_panels"][number]; items: DepartmentModuleItem[]; settings: FoundationSettings; weather: WeatherPayload | null; lodd: LoddPayload | null; sourceData: StickneyModuleData | null; departmentSlug: string; refreshKey: number; index: number; onSelect: (index: number) => void }) {
  return <div className="live-card"><h3>{panelLabels.get(panel) || "Station information"}</h3><div className="live-rotation-body"><PanelContent departmentId={departmentId} panel={panel} items={items} settings={settings} weather={weather} lodd={lodd} sourceData={sourceData} departmentSlug={departmentSlug} refreshKey={refreshKey}/></div><div className="live-rotation-dots">{settings.live_board_panels.map((key, panelIndex) => <button type="button" className={panelIndex === index % settings.live_board_panels.length ? "active" : ""} key={key} onClick={() => onSelect(panelIndex)} aria-label={`Show ${panelLabels.get(key)}`}/>)}<span>Rotates every {settings.board_rotation_seconds} seconds</span></div></div>;
}

function PanelContent({ departmentId, panel, items, settings, weather, lodd, sourceData, departmentSlug, refreshKey }: { departmentId: string; panel: FoundationSettings["live_board_panels"][number]; items: DepartmentModuleItem[]; settings: FoundationSettings; weather: WeatherPayload | null; lodd: LoddPayload | null; sourceData: StickneyModuleData | null; departmentSlug: string; refreshKey: number }) {
  ...
  return weather?.configured ? <OfficialRadar key={refreshKey} departmentId={departmentId} sourceUrl={settings.live_board_radar_url} refreshKey={refreshKey} title="Live selected-area weather radar"/> : <EmptySource title="Weather radar location not configured" text="Save verified latitude, longitude coordinates in Department settings. A radar webpage link alone is not embedded."/>;
}
...
function OfficialRadar({ departmentId, sourceUrl, refreshKey, title }: { departmentId: string; sourceUrl: string; refreshKey: number; title: string }) {
  const [failed, setFailed] = useState(false);
  const imageUrl = `/api/departments/${encodeURIComponent(departmentId)}/radar?refresh=${refreshKey}`;
  return <div className="live-official-radar">
    <div className="live-radar-map">
      {!failed ? <img key={imageUrl} src={imageUrl} alt={title} onError={() => setFailed(true)}/> : <div className="live-radar-missing"><b>Official radar is temporarily unavailable</b><span>Use Open configured source and verify current conditions before operational use.</span></div>}
      <div className="live-radar-grid" aria-hidden="true"/><div className="live-radar-center" aria-hidden="true"><i/><span>DEPARTMENT</span></div>
    </div>
    <div className="live-radar-caption"><span>NOAA / NWS MRMS base reflectivity · latest available image</span><div><a href="https://radar.weather.gov/" target="_blank" rel="noreferrer">Open NOAA radar</a>{sourceUrl ? <a href={sourceUrl} target="_blank" rel="noreferrer">Open configured source</a> : null}</div></div>
  </div>;
}
...
function RadarOverlay({ departmentId, takeover, radarUrl, refreshKey, clock, onClose }: { departmentId: string; takeover: RadarTakeover; radarUrl: string; refreshKey: number; clock: Date | null; onClose: () => void }) {
  const remaining = Math.max(0, Math.ceil((takeover.endsAt - (clock?.getTime() ?? takeover.endsAt)) / 1000));
  return <div className={`live-radar-takeover ${takeover.kind === "severe" ? "severe" : ""}`} data-incident-priority="respond" role="alert" aria-live="assertive"><header><div><span>{takeover.kind === "severe" ? "WEATHER WARNING · PRIORITY DISPLAY" : "WEATHER RADAR"}</span><h2>{takeover.title}</h2><p>{takeover.detail}</p></div><div><b>{remaining}s</b><button type="button" onClick={onClose}>Return to board</button></div></header><div className="live-radar-frame"><OfficialRadar key={refreshKey} departmentId={departmentId} sourceUrl={radarUrl} refreshKey={refreshKey} title={`${takeover.title} radar`}/></div></div>;
}
