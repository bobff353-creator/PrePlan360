...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string; photoId: string }> }) {
  ...
  const media = await fermilabMedia("inventory_media", photoId);
  if (!media) return new Response("Inventory photo not found", { status: 404 });
  if (!media.contentType.startsWith("image/")) return new Response("Inventory photo unavailable", { status: 502 });
